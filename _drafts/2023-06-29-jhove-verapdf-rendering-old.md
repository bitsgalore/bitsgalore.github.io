...
- Tool reports that are difficult to interpret.
...
He also argues that validation is often treated as an end in itself, without much consideration of the purpose it is supposed to fulfil. 

As a way out of this situation, he stresses that it's vital to first clearly state "what we actually want to achieve in a risk assessment of digital content". He suggests the overall purpose here is to answer the following questions: 
...
The risk assessment should then ensure that a file is not encypted, doesn't have any extenal dependencies, isn't badly broken, and its format is indeed the expected format[^8]. Even though this won't 100% guarantee that a file will render, at least it covers the most likely obstacles against a file successfully rendering.

## Role of validation

Within this risk assessment, Paul largely views validation as an automated proxy for rendering a file in a viewer (which would be too laborious to do manually). The primary purpose of the validation tool is then to "highlight the (hopefully) small number of our files that warrant further investigation via manual assessment".


Wheatley suggests digipres community drops interest in validation, and instead invest in adapting rendering tools:
...
My [previous post]({{ BASE_PATH }}/2023/05/25/identification-of-pdf-preservation-risks-with-verapdf-and-jhove) addressed the use of [JHOVE](https://jhove.openpreservation.org/) and [VeraPDF](https://verapdf.org/) for identifying preservation risks in PDF files. In the concluding section, I suggested that VeraPDF's parse status might be used as a rough "validity proxy" to identify malformed PDFs. This made me think about ways to back up that suggestion with some evidence. This post is a first attempt at this, using data from the [Synthetic PDF Testset for File Format Validation](https://www.radar-service.eu/radar/en/dataset/JtlOdwQquZWDqQdq) by Lindlar, Tunnat and Wilson.

<!-- more -->

## What about JHOVE?

Some readers may wonder what's even the point of exploring this subject, since we already have a full-fledged PDF validator within JHOVE. This is partially true, but JHOVE, and in particular its PDF module, has some serious shortcomings. [This 2017 paper by Lindlar and Tunnat](https://doi.org/10.2218/ijdc.v12i2.578)
addresses several of them. 

### Interpretation of validation errors

In particular, the authors mention the often inpenetrable validation error messages, which make it hard to assess if any particular error has an impact on rendering and long-term accessibility. 

To be fair, this situation has improved somewhat since 2017. For example, there have been several initiatives to document JHOVE's validation errors, and the results are available from JHOVE's Wiki pages ([here](https://github.com/openpreserve/jhove/wiki/PDF-hul-Messages) and [here](https://github.com/openpreserve/jhove/wiki/PDF-hul-Messages-2)). These pages cover a total of 158 PDF-specific validation messages. However, out of these, the actual *impact* is documented for only 14 messages. Out of these 14:

- Six indicate no impact at all.
- Three indicate an impact that is either minor, "not large" or unknown.
- Five indicate a major impact.

This means that the actual impact is still unknown for the vast majority of JHOVE's PDF validation errors, which makes it hard to judge how serious any particular validation error actually is.

### Technical debt

This situation is exacerbated by the fact that JHOVE's codebase is affected by years of technical debt. For example, it still [doesn't support PDF 1.7](https://jhove.openpreservation.org/modules/pdf/), which has been around since 2006. This means that JHOVE may falsely see PDFs that contain features that were introduced in PDF 1.7 as "not valid", even though they conform to the 1.7 specification. PDF 2.0 (published in 2017) isn't supported either, and the tests in [my previous post]({{ BASE_PATH }}/2023/05/25/identification-of-pdf-preservation-risks-with-verapdf-and-jhove) showed that JHOVE flat-out refuses to parse any PDF 2.0 files altogether! This is especially poignant since PDF 1.7 is the first PDF version that was published as an ISO standard. Most (recent versions of) ISO-standardised PDF profiles such as PDF/A and PDF/UA are subsets of these ISO-standardised PDF versions (ISO 32000-1 for PDF 1.7, and ISO 32000-2 for PDF 2.0). It's unfortunate that JHOVE's output cannot be trusted for these modern PDF variants.

## Goal and objectives of this post

The main goal of this post is to get more insight into the associations between VeraPDF parse errors and rendering behaviour, and to see if the occurrence of these parse errors can be used as rough "validity proxy" to identify malformed PDFs. For this I propose a tentative methodology, and apply this to a previously published annotated dataset of synthetic PDF files. I then try to use the results to answer the following questions:

1. To what extent is VeraPDF's reported parse status associated with rendering behaviour in Adobe Acrobat?
2. How does this compare with JHOVE?
3. What are the implications of the answers to 1. and 2. for preservation workflows?

In the closing section I also offer some thoughts on the future of PDF validation. This was partially prompted by the release of a development preview of a new veraPDF-powered PDF-checker that is based on the [Arlington PDF model](https://github.com/pdf-association/arlington-pdf-model).
...
Out of the 32 files in the "Does not render" category, JHOVE considers 31 as "Not well-formed", and 1 as "Well-Formed, but not valid". JHOVE was less successful at identfying files in the "Renders with issues" category. Out the 38 files, 9 (24%) are "Well-Formed and valid". One striking result is that from the 18 files in the "Renders normally" category, the majority (11, or 61%) are "Not well-formed" according to JHOVE. 
...
Whether this behaviour is desirable or not all highly depends the specific purpose for which we want to "validate". If the objective is to identify PDFs that are seriously malformed, VeraPDF's parse status provides a simple and most likely sufficient indicator. JHOVE's validation output gives more detail, but its interpretation can be difficult for the reasons explained at the start of this post. For PDFs with features from PDF 1.7 or later, JHOVE's output may be outright misleading. 
...
## Arlington model: the future of PDF validation?

As I'm finishing this write-up, the Open Preservation Foundation and the PDF Association have just [released](https://openpreservation.org/news/development-preview-pdf-file-checker-based-on-the-arlington-pdf-model/) a first development preview of a new veraPDF-powered PDF-checker that is based on the [Arlington PDF model](https://github.com/pdf-association/arlington-pdf-model). This software is capable of analyzing PDF files against the full PDF 2.0 (and earlier) specifications. This is not 100% identical to "JHOVE style" PDF validation, because the Arlington model currently doesn't define PDF lexical rules and dialects, content streams, and file stucture and layout rules. These limitations aside, the description suggests it comes *pretty close* to full PDF validation. Unlike JHOVE, the VeraPDF codebase is up-to-date with the latest PDF standards, and it isn't plagued by years of technical debt. Moreover, it is developed and maintained by a consortium that includes the [PDF Association](https://pdfa.org/), which is also involved in writing the actual PDF standards. This has resulted in a much better alignment of the tool with the standards, compared to JHOVE.

I haven't tested the Arlington-based PDF checker myself yet, but on first impression this looks like the future of PDF validation to me. As a digital preservation community, I think the time has come to reconsider the time and effort we put into improving, documenting and researching PDF validation in JHOVE. VeraPDF looks like an alternative that could be much more viable long-term, and the Arlington-based PDF validation has the potential to make JHOVE's PDF validation obsolete. So why not redirect our collective resources towards making that happen? In the meantime, VeraPDF's reporting on the [presence of preservation risks]({{ BASE_PATH }}/2023/05/25/identification-of-pdf-preservation-risks-with-verapdf-and-jhove), combined with the occurrence of parse errors already provides a wealth of information about 
...
- The future of PDF validation, and the roles of JHOVE, VeraPDF and the Arlington model.

Feel free to use either the comment section, Mastodon, e-mail, or write your own reaction post.
...
- [The Arlington PDF Model](https://github.com/pdf-association/arlington-pdf-model)

- [Development Preview: PDF file checker based on the Arlington PDF Model](https://openpreservation.org/news/development-preview-pdf-file-checker-based-on-the-arlington-pdf-model/)
